...
export function findNxNy(input) {
  ...
  // 2-b) 동-숫자 확장이 전무하면, 도시 범위 안에서 "동 어간 포함" 키를 긁어서 최빈 좌표 선택
  const stemNorm = norm(dongStem); // ex) '개금'
  const containHits = pool.filter(k => {
    const nk = norm(k);            // 예: '부산광역시부산진구개금1동'
    return nk.includes(stemNorm);  // '개금'이 들어가는 모든 키 수집
  });
