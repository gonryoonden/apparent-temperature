...
export function resolveRegion(userText) {
  ...
  if (aliases[userText]) {
    ...
    if (as && as.length) {
      ...
      if (resolvedAdmin) return { ok:true, match:'alias->legal->admin', legalKey, adminKey: resolvedAdmin, nxny:nxny[resolvedAdmin] };
    }
  }
  ...
  if (siDo && siGunGu) {
    ...
    const resolvedAdmin = resolveAdminKey(adminKey);
    if (resolvedAdmin) return { ok:true, match:'admin-exact', adminKey: resolvedAdmin, nxny:nxny[resolvedAdmin] };
  }
  ...
  if (siDo && !siGunGu) {
    ...
    for (const c of cities) {
      ...
      if (as && as.length) {
        const adminKey = as[0];
        const resolvedAdmin = resolveAdminKey(adminKey);
        if (resolvedAdmin) return { ok:true, match:'legal->admin', legalKey, adminKey: resolvedAdmin, nxny:nxny[resolvedAdmin] };
      }
    }
  }
  ...
  if (siDo && siGunGu) {
    ...
    if (candidates.length === 1) {
      ...
      const resolvedAdmin = resolveAdminKey(adminKey);
      if (resolvedAdmin) {
        return { ok:true, match:'auto-corrected-gu', legalKey: candidates[0].legalKey, adminKey: resolvedAdmin, nxny:nxny[resolvedAdmin] };
      }
    }
  }
